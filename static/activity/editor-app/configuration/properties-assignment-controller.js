...
var KisBpmChoseAssignmentCtrl = ['$scope', '$http', function($scope, $http) {
    ...
    $scope.getAllRoles = function (successCallback) {
        $http({
            ...
            url: 'https://dev.zdsxc.com/activiti/act/showRoleList'})
    ...
    function getDetail(value,page,limit)
    {
        ...
        $http({
            ...
            headers: {'Accept': 'application/json',
                'Content-Type': 'application/json; charset=UTF-8'},
            url: 'https://dev.zdsxc.com/activiti/act/listByRoleId?roleId='+value+"&current="+page+"&size="+limit})
            .success(function (data, status, headers, config) {
                //封装数据
                var obj = data.list;
                var accounts = [];
                if (obj&&obj.length>0) {
                    for (var i=0; i<obj.length; i++) {
                        accounts.push({id:obj[i].id, code : obj[i].id, name : obj[i].name, index:i});
                    }
                }
...
var KisBpmChoseCandidateGroupsCtrl = ['$scope', '$http', function($scope, $http) {
    ...
    $scope.save = function() {
        ...
        choseAssigneesNameStr = choseAssigneesNameStr.substring(0,choseAssigneesNameStr.length-1);
        console.log(choseCandidateGroupsStr)
        console.log(choseAssigneesNameStr)
