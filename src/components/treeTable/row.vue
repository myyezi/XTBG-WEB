<template>
                ...
                <column
                    ...
                    <span v-else-if="subItem.type === 'action'">
                        <a class="action-item"
                            ...
                            :class="{'is_gray':(acItem.disabledType==1?(model.children&&model.children.length>0):acItem.disabledType==2?(model[custom_field.is_last]===1):false)}"
                            @click.stop.prevent="acItem.disabledType==1?(model.children&&model.children.length>0)?'':acItem.onclick(model):acItem.disabledType==2?(model[custom_field.is_last]===1)?'':acItem.onclick(model):acItem.onclick(model)">
                    ...
                    <span v-else @click.stop.prevent="subItem.isCellsClick?subItem.cellsClick(model):''">
                        <span v-if="subItem.formatter" v-html="subItem.formatter(model)"></span>
                        <span v-else :title="model[subItem.field]">{{model[subItem.field]}}</span>
                    </span>
                </column>
    ...
    export default {
        name: 'row',
        props: ['model','depth','columns','isdraggable','isopen','custom_field','onCheck','isContainChildren'],
        data() {
            return {
                open: false,
                visibility: 'visible'
        ...
        methods: {
            ...
            onCheckboxClick(evt, model) {
              ...
              // 判断是否有子节点，如有需递归处理
              if (list && list.length>0 && this.isContainChildren) {
