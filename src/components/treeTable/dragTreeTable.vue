    ...
    <div class="drag-tree-table" ref="table">
        <div class="drag-tree-table-header">
          <column
            ...
            <div v-else class="drag-tree-table-header_title">
              <span> {{item.title}} </span>
              <img :src='item.img'  v-if="item.img" @click.stop.prevent="item.onclick()"/>
            </div>
          </column>
        </div>
        <div class="drag-tree-table-body"
          ...
          <row depth="0" :columns="data.columns" :isdraggable="isdraggable" :isopen="isopen"
            :model="item"
  ...
  export default {
    ...
    methods: {
      ...
      getCheckedList(lists) {
        ...
        getchild(deepList)
        this.titleChecked = isCheckedFlag
        return checkedList;
      },
    },
    mounted() {
