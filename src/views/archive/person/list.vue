    ...
    export default {
        ...
        methods: {
            ...
            onAdd(data) {
                ...
                this.archivePersonWJForm.file=[];
                console.log(this.archivePersonWJForm)
            ...
            deletePersionArchive(data,node) {
                ajax.get('archive/person/remove',{
                    id:data.id,
                    type:data.isLast
                }).then(rs => {
                    this.getArchivePersonTree();
                    ...
                    if (valid) {
                        ...
                        // 弹窗表单修改的字段需要给formData中对应的字段附值
                         ...
                         if(this.formData.isLast===1){
                             this.addarchivePersonDetail(newChild)
                         }else{
                             this.addarchivePerson(newChild);
                         }
                    } else {
                        return false;
                    }
            ...
            // 获取树形结构
            getArchivePersonTree() {
                ajax.get('archive/person/getArchivePersonTree',{name:this.name}
                ).then(rs => {
                    if(rs.status === 0) {
                        if(rs.data) {
